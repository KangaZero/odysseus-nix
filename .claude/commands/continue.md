...
- If `flake.nix` needs changes, make them and run `just test` to verify.
...
- **Version bumps**: is `python314` still current? Is there a newer LTS Node in nixpkgs (`nodejs_22` → `nodejs_24`)?
- **nixpkgs channel**: is `nixos-unstable` appropriate, or would `nixos-25.05` be more stable for this use case? (note: `python314` requires unstable — switching would downgrade to 3.13)
